...
import org.schabi.newpipe.player.helper.PlayerHelper.MinimizeMode.Companion.MINIMIZE_ON_EXIT_MODE_BACKGROUND
import org.schabi.newpipe.player.helper.PlayerHelper.MinimizeMode.Companion.MINIMIZE_ON_EXIT_MODE_NONE
import org.schabi.newpipe.player.helper.PlayerHelper.MinimizeMode.Companion.MINIMIZE_ON_EXIT_MODE_POPUP
...
object PlayerHelper {
    ...
    fun getTimeString(milliSeconds: Int): String {
        ...
        return if (days > 0)
            stringFormatter.format("%d:%02d:%02d:%02d", days, hours, minutes, seconds).toString()
        else if (hours > 0)
            stringFormatter.format("%d:%02d:%02d", hours, minutes, seconds).toString()
        else
    ...
    fun mimeTypesOf(format: SubtitlesFormat): String {
        when (format) {
            SubtitlesFormat.VTT -> return MimeTypes.TEXT_VTT
            SubtitlesFormat.TTML -> return MimeTypes.APPLICATION_TTML
    ...
    fun resizeTypeOf(context: Context,
        ...
        when (resizeMode) {
            RESIZE_MODE_FIT -> return context.resources.getString(R.string.resize_fit)
            RESIZE_MODE_FILL -> return context.resources.getString(R.string.resize_fill)
            RESIZE_MODE_ZOOM -> return context.resources.getString(R.string.resize_zoom)
    ...
    fun getSeekParameters(context: Context): SeekParameters {
        return if (isUsingInexactSeek(context))
